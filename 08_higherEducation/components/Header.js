...
const customSelectSizeElement = document.getElementById(
  "custom-select-size"
);
...
const optionsListSizeElement = document.getElementById("options-list-size");
...
customSelectSizeElement.addEventListener("click", (event) => {
  // Check if the click target is not a checkbox inside the customSelectTypeElement
  if (
    !event.target.closest(".form-checkbox") &&
    !event.target.closest("label")
  ) {
    optionsListSizeElement.classList.toggle("invisible");
  }
});
...
document.addEventListener("click", (event) => {
  ...
  if (
    !customSelectSizeElement.contains(event.target) &&
    !optionsListSizeElement.contains(event.target)
  ) {
    optionsListSizeElement.classList.add("invisible");
  }
});
