...
const addToCalculatedRow = (year, data, name, type, fixedNum) => {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    console.log(data[`cfi_${name}_Client`][year])

    if (i === 0) {
      // console.log(`cfi_${name}_Client`);
      const value = data[`cfi_${name}_Client`][year].value;
    } else if (i === 1) {
      // console.log(`cfi_${name}_Strength_Client`);
      const value = data[`cfi_${name}_Strength_Client`][year].value;
    } else if (i === 2) {
      const value = data[`cfi_${name}_Weight_Client`][year].value;
    } else if (i === 3) {
      const value = data[`cfi_${name}_Score_Client`][year].value;
    } 
