...
const displayFinancialStatementComponent = () => {
  ...
  const assetsChart = new ApexCharts(
    ...
    getFSchartOptions(parseData, 'totalAssets_Client', window.chartColors.green)
  );
  assetsChart.render();
  document.addEventListener("dark-mode", function () {
    assetsChart.updateOptions(getFSchartOptions(parseData, 'totalAssets_Client', window.chartColors.green));
  });
};
