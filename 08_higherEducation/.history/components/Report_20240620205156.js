const displayReportComponent = () => {
  ...
  if (selectedYears) {
    ...
    insertDataToReport (cfiData, selectedYears, 'row_cfiRatio_tableHeader' [
      ['cfiRatio', 'num', 1],
    ...
    insertDataToReport (cfiData, selectedYears, 'row_primaryReserveRatio_tableHeader' [
      ['nonrestrictedNetAssets', 'dollar', 0],
      ['restrictedNetAssets', 'dollar', 0],
      ['propertyAndEquipmentNet', 'dollar', 0],
...
const insertCalculatedDataToReport = (data, selectedYears, arrayOfNames) => {
  ...
  if (data && selectedYears) {
    ...
    const ratioValue = data['row_cfiRatio_Client'] && data['row_cfiRatio_Client'][year] ? data['row_cfiRatio_Client'][year].value : '-';
    // console.log({ ratioValue });
    const thCfiScoreElement = document.getElementById('th_cfiScore');
    thCfiScoreElement.textContent = (ratioValue !== undefined && !isNaN(ratioValue) && ratioValue !== 0) ? ratioValue : '-';
...
const addToCalculatedRow = (year, data, name, type, fixedNum) => {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const value = data[field] && data[field][year] ? data[field][year].value : undefined;
...
const insertDataToReport = (data, selectedYears, tableHeaderRow arrayOfNames) => {
  if (data && selectedYears) {
    addTotalDataToEveryRow (data, selectedYears, arrayOfNames, tableHeaderRow);
  }
};
