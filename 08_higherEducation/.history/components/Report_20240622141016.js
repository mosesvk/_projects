...
const insertDataToAssetToLiabilityReport = (data, selectedYears) => {
  ...
  selectedYears.forEach(year => {
      ...
      const totalAssetToLiabilitysPeerValue = totalAssetsPeerValue / totalLiabilitiesPeerValue;

      const peerRow = document.createElement('tr');
      peerRow.innerHTML = `
          <th scope="row" class="px-6 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">${year}</th>
          <td class="px-6 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">${styleNumber(totalAssetsPeerValue, 'dollar', 0)}</td>
          <td class="px-6 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">${styleNumber(totalLiabilitiesPeerValue, 'dollar', 0)}</td>
          <td class="px-6 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">${styleNumber(totalAssetToLiabilitysPeerValue, 'dollar', 0)}</td>
      `;
