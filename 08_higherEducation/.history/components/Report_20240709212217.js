const displayReportComponent = () => {
  ...
  console.log({selectedYears.sort((a, b) => b - a)});
...
const insertDataToFfaReport = (data, selectedYears) => {
  ...
  document.getElementById ('ffa_tuitionFees').textContent = revenueTuitionAndFeesClient > 0
  ...
  document.getElementById ('ffa_scholarshipsFinancial').textContent = revenueSchoolServicesClient > 0
  ...
  document.getElementById ('ffa_unrestrictedGifts').textContent = totalRevenueContributionsClient > 0
  ...
  document.getElementById ('ffa_auxiliaryOther').textContent = auxiliaryAndOtherClient > 0
  ...
  document.getElementById ('ffa_restrictedGifts').textContent = contributionsClient > 0
  ...
  document.getElementById ('ffa_compensationBenefits').textContent = compensationAndBenefitsClient > 0
  ...
  document.getElementById ('ffa_generalExpense').textContent = generalExpenseClient > 0
    ? styleNumber (generalExpenseClient, 'dollar', 0)
    : '-';

  const totalRevenues = revenueTuitionAndFeesClient + totalRevenueContributionsClient + auxiliaryAndOtherClient + contributionsClient
  const totalExpenses = revenueSchoolServicesClient + compensationAndBenefitsClient  + generalExpenseClient

  document.getElementById ('ffa_totalRevenues').textContent = totalRevenues > 0
    ? styleNumber (totalRevenues, 'dollar', 0)
    : '-';
  document.getElementById ('ffa_totalExpenses').textContent = totalExpenses > 0
    ? styleNumber (totalExpenses, 'dollar', 0)
    : '-';
}
...
const insertDataToSourceOfInomeReport = (data, selectedYears) => {
  ...
  variables.forEach ((variable, index) => {
    ...
    const peerValue = data[peerName][selectedYears[0]].length > 0
      ...
      const classTrEven = 'bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600'
      const classTrOdd = 'backgroundOffGreen border-b dark:bg-gray-700 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600'
...
const insertDataToAssetToLiabilityReport = (data, selectedYears) => {
  ...
  const tableBodyPeer = document.getElementById (
    'assetToLiabilitiesPeer_tbody'
  );

  // console.log({ totalAssetsPeer, totalLiabilitiesPeer });

  selectedYears.forEach ((year, index) => {
    const totalAssetsClientValue = Number (totalAssetsClient[year].value) > 0
      ? styleNumber (totalAssetsClient[year].value, 'dollar', 0)
      : '-';
    const totalLiabilitiesClientValue = Number (
      totalLiabilitiesClient[year].value
    ) > 0
      ? styleNumber (totalLiabilitiesClient[year].value, 'dollar', 0)
      : '-';
    const totalAssetToLiabilityClientValue = Number (
      totalAssetsClient[year].value
    ) > 0
      ? styleNumber (
    ...
    const classTrClient = index % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'backgroundOffGreen dark:bg-gray-700';
    ...
    const totalAssetsPeerValue = Number (
      getSumOfArray (totalAssetsPeer[year])
    ) > 0
      ? styleNumber (getSumOfArray (totalAssetsPeer[year]), 'dollar', 0)
      : '-';
    const totalLiabilitiesPeerValue = Number (
      getSumOfArray (totalLiabilitiesPeer[year])
    ) > 0
      ? styleNumber (getSumOfArray (totalLiabilitiesPeer[year]), 'dollar', 0)
      : '-';
    const ratioPeer = Number (getSumOfArray (totalLiabilitiesPeer[year])) > 0
      ? Number (getSumOfArray (totalAssetsPeer[year])) /
          Number (getSumOfArray (totalLiabilitiesPeer[year]))
      : 0;
    const totalAssetToLiabilitysPeerValue = ratioPeer > 0
      ? styleNumber (ratioPeer, 'num', 2)
      : '-';

    const classTrPeer = index % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'backgroundOffBlue dark:bg-gray-700';
