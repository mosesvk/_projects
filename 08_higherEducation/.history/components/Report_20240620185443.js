const displayReportComponent = () => {
  ...
  if (selectedYears) {
    ...
    insertDataToReport (cfiData, selectedYears, [
      ['cfiRatio', 'num', 1],
    ...
    insertCalculatedDataToReport (cfiData, selectedYears, [
      ['cfiScore_primaryReserveRatio', 'num', 2],
      ['cfiScore_netIncomeOperationsRatio', 'percent', 1],
      ['cfiScore_returnOnNetAssets', 'percent', 1],
      ['cfiScore_viabilityRatio', 'num', 2],
    ]);
...
const insertCalculatedDataToReport = (data, selectedYears, arrayOfNames) => {
  if (data && selectedYears) {
    for (let name of arrayOfNames) {
      addToCalculatedRow (selectedYears[0], data, name[0], name[1], name[2]);
    }
  }
};
...
const addToCalculatedRow = (year, data, name, type, fixedNum) => {
  ...
  // console.log({tableReportRow, year, data, name, type, fixedNum, row: `row_${name}`})  
  ...
  for (let i = 0; i < 4; i++) {
    ...
    console.log({client, tableRow, year, type, fixedNum, dataPoint, data})


    if (i === 0) {
      const value = data[`${name}_Client`].year.value;
    } else if (i === 1) {
      const value = data[`${name}_Strenth_Client`].year.value;
    } else if (i === 2) {
      const value = data[`${name}_Weight_Client`].year.value;
    } else if (i === 3) {
      const value = data[`${name}_Score_Client`].year.value;
    } 
