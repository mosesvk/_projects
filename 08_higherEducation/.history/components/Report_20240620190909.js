...
const addToCalculatedRow = (year, data, name, type, fixedNum) => {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (i === 0) {
      ...
      const value = data[`cfiScore_${name}_Strenth_Client`].year.value;
    } else if (i === 2) {
      const value = data[`cfiScore_${name}_Weight_Client`].year.value;
    } else if (i === 3) {
      const value = data[`cfiScore_${name}_Score_Client`].year.value;
    } 
