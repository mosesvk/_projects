...
const getDebtBurdenRatioChartOptions = (data) => {
  ...
  selectedYearsArray.map((year) => {
    const clientRatioNum = Math.abs((Number(data.ratio_Client[year].value) * 100).toFixed(1));
    const debtServiceNum = Math.abs(Number(data.debtService_Client[year].value))
    ...
    const peerRatioNum = getSumOfArray(data.ratio_Peer[year])
