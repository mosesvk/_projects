...
const getTuitionDependencyChartOptions = (data) => {
  ...
  selectedYearsArray.map((year) => {
    ...
    const netTuitionAndFees = Math.round(Number(data.netTuitionAndFees_Client[year].value));
    netTuitionAndFeesArray.push(netTuitionAndFees);

    const operatingRevenuesSupportAndRelease = Math.round(
      Number(data.operatingRevenuesSupportAndRelease_Client[year].value)
    );
    operatingRevenueArray.push(operatingRevenuesSupportAndRelease);
  });
