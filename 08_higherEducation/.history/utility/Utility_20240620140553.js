...
const styleNumber = (num, type, fixed) => {
  ...
  if (!isNaN (text)) {
    if (type === 'num' && text != 0) {
      ...
      console.log ({textNum, text, fixed});
      text = Number (textNum).toLocaleString (); // Add commas for thousands
      if (fixed > 0 && textNum % 1 === 0) {
        // If 'fixed' is greater than 0 and 'textNum' is a whole number,
...
function missionaryRange () {
  return {
    ...
    missiontrigger () {
      ...
      this.missionthumb = missionValuePercent > 100
        ? 100
        : missionValuePercent < 0 ? 0 : missionValuePercent;
    },
  };
}
...
const getBackgroundColor = (array, row, i = 1) => {
  ...
  let color = array[0] === 'Warning'
    ? 'warning'
    : array[0] === 'Good'
        ? 'good'
        : array[0] === 'Action Required' ? 'actionRequired' : null;
