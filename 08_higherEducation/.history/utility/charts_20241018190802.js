...
const getTuitionDiscountRateChartOptions = (data) => {
  ...
  selectedYearsArray.map((year) => {
    ...
    const scholarshipsAndFinancialAidClient = Math.round(Number(data.scholarshipsAndFinancialAid_Client[year].value));
    scholarshipArray.push(scholarshipsAndFinancialAidClient);

    const tuitionAndFeesClient = Math.round(
      Number(data.tuitionAndFees_Client[year].value)
    );
