...
const getFpaChartOptions = (
  ...
  const totalAssetsArray = Object.values(data['totalAssets_Client']).map(item => item.value).reverse();
  const totalLiabilitiesArray = Object.values(data['totalLiabilities_Client']).map(item => item.value).reverse();
  const netPositionArray = Object.values(data['netPosition_Client']).map(item => item.value).reverse();
