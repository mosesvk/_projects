...
function createFSTable(tableDataClass, data) {
  console.log({ tableDataClass, data });
...
function processFinancialData(dataObject, tableId) {
  // Retrieve the data object from localStorage
  // let dataObject = JSON.parse(localStorage.getItem(dataName));

  // Get the most current year from the processSelectedYears function
  let years = processSelectedYears();
  let currentYear = Math.max(...years);

  // Create an array of values for the current year
  let arrayData = [];
  for (let key in dataObject) {
    if (dataObject[key][currentYear]) {
      arrayData.push(dataObject[key][currentYear].value);
    }
  }

  // Call the createFSTable function with the tableId and arrayData
  createFSTable(tableId, arrayData);
}
...
function createAndRenderFSChart(
  ...
  const chart = new ApexCharts(
    ...
    getFSchartOptions(parsedData, dataKey, color, currency, label, chartId)
  );
  chart.render();
