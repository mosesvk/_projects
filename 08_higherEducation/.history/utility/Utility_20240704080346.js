...
const getPeerAndClientChartDataArrays = (
  ...
  years.forEach((year) => {
    if (mainName == 'cfi_primaryReserveRatio') console.log({year, peer: dataPeer[year]})
...
const updateCfiValue = (cfiValue) => {
  ...
  for (let i = 0; i < thresholds.length; i++) {
    ...
    if (cfiValue === thresholds[i]) {
      document.getElementById(thresholds[i-1].toFixed(1)).innerHTML = cfiValue;
      document.getElementById(thresholds[i-1].toFixed(1)).classList = "font-bold text-lg"
    }
  }
