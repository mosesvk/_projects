...
const getTuitionDiscountRateChartOptions = (data) => {
  ...
  selectedYearsArray.map((year) => {
    ...
    peerRatioArray.push(ratioPeer);
    const scholarshipsAndFinancialAidPeer = revenueScholarshipsAndFinanancialAid_Peer[year] ? Math.abs(
      Math.round(
        getAverageOfArray(data.revenueScholarshipsAndFinanancialAid_Peer[year])
      )
    ) : 0
    const tuitionAndFeesPeer = revenueTuitionAndFees_Peer[year] ? Math.round(
...
const getEndowmentAssetsPerStudentChartOptions = (data) => {
  ...
  selectedYearsArray.map((year) => {
    ...
    const endowmentSizePeer = endowment_Peer[year] ? getSumOfArray(data.endowment_Peer[year]): 0
    const totalStudentFtePeer = totalStudentFte_Peer[year] ? getSumOfArray(data.totalStudentFte_Peer[year]) : 0
