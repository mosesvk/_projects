...
// Example usage:
const svgElement = "<svg xmlns='http://www.w3.org/2000/svg' width='100' height='100'><circle cx='50' cy='50' r='40' fill='red' /></svg>";
const pngBase64Code = await svgToPngBase64(svgElement);
console.log(pngBase64Code); // Use the base64 code as needed



function initializeChart(mainName, chartId, dataPeer, dataClient, type, fixedNum, benchmark, title) {
  if (mainName === "cfiRatio") {
    const chart = new ApexCharts(
      document.getElementById(chartId),
      getMainChartOptions(dataPeer, dataClient, type, fixedNum, mainName, benchmark, title)
    );

    chart.render();

    document.addEventListener("dark-mode", function () {
      chart.updateOptions(getMainChartOptions(dataPeer, dataClient, type, fixedNum, mainName, benchmark, title));
    });
  }
}
