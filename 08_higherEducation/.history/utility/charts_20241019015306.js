...
const getTuitionDiscountRateChartOptions = (data) => {
  ...
  selectedYearsArray.map((year) => {
    const ratioClient = Math.abs(Math.round(Number(data.ratio_Client[year].value) * 100))
    clientRatioArray.push(ratioClient);

    const scholarshipsAndFinancialAidClient = Math.abs(Math.round(Number(data.revenueScholarshipsAndFinanancialAid_Client[year].value)))
    ...
    const ratioPeer = Math.abs(Math.round(getAverageOfArray(data.ratio_Peer[year], 100)))
    peerRatioArray.push(ratioPeer);
    const scholarshipsAndFinancialAidPeer = Math.abs(Math.round(getAverageOfArray(data.revenueScholarshipsAndFinanancialAid_Peer[year])))
