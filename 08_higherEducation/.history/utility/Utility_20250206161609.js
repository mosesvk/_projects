...
const formatWithFixed = (number, type, fixedNum) => {
  // Determine if the number is an integer and if fixedNum is 1
  const isIntegerWithFixedOne = Number.isInteger(number) && fixedNum === 1;

  // Format the number based on the type
  let formattedNumber = isIntegerWithFixedOne
    ? number.toFixed(1)
    : number.toFixed(fixedNum);

  // Add the appropriate symbol based on the type
  switch (type) {
    case 'dollar':
