  ...
  const chartEvents = {
    beforeMount: function (chartContext, config) {
      setTimeout(function () {
        ...
        const gridLines = chartElement.querySelectorAll(
          ".apexcharts-gridlines-horizontal line"
        );
        if (!gridLines.length) return;

        // Get the first grid line to determine width
        const firstGridLine = gridLines[0];
        const x1 = parseFloat(firstGridLine.getAttribute("x1"));
        const x2 = parseFloat(firstGridLine.getAttribute("x2"));
