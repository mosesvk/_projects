...
const getAtlChartOptions = data => {
  ...
  console.log ({totalAssetsClient, totalLiabilitiesClient, totalAssetsPeer, totalLiabilitiesPeer});
...
const getFfaChartOptions = data => {
  ...
  const revenueTuitionAndFeesClient = Number(data['ffa_revenueTuitionAndFees_Client'][currentYear].value)
  const revenueSchoolServicesClient = Number(data['ffa_revenueScholarshipsAndFinancialAid_Client'][currentYear].value)
  const totalRevenueContributionsClient = Number(data['ffa_revenueContributions_Client'][currentYear].value)

  const revenueAuxiliaryActivitiesClient = Number(data['ffa_revenueAuxiliaryActivities_Client'][currentYear].value)
  const revenueOtherClient = Number(data['ffa_revenueOther_Client'][currentYear].value)
  const revenueInvestmentIncomeClient = Number(data['ffa_revenueInvestmentIncome_Client'][currentYear].value)
  const revenueEndowmentSpendingAppropriationClient = Number(data['ffa_revenueEndowmentSpendingAppropriation_Client'][currentYear].value)
  const auxiliaryAndOtherClient = revenueAuxiliaryActivitiesClient + revenueOtherClient + revenueInvestmentIncomeClient + revenueEndowmentSpendingAppropriationClient

  const contributionsClient =  Number(data['ffa_contributions_Client'][currentYear].value)

  const salariesAndWagesClient = Number(data['ffa_salariesAndWages_Client'][currentYear].value)
  const employeeBenefitsClient = Number(data['ffa_employeeBenefits_Client'][currentYear].value)
  const compensationAndBenefitsClient = salariesAndWagesClient + employeeBenefitsClient

  const servicesSuppliesAndOtherClient = Number(data['ffa_servicesSuppliesAndOther_Client'][currentYear].value)
  const occupancyUtilitiesAndMaintenanceClient = Number(data['ffa_occupancyUtilitiesAndMaintenance_Client'][currentYear].value)
  const depreciationAndAmortizationClient = Number(data['ffa_depreciationAndAmortization_Client'][currentYear].value)
  const interestClient = Number(data['ffa_interest_Client'][currentYear].value)
  const incomeExpenseSurplusDefecitClient = Number(data['ffa_incomeExpenseSurplusDefecit_Client'][currentYear].value)
  const generalExpenseClient = servicesSuppliesAndOtherClient + occupancyUtilitiesAndMaintenanceClient + depreciationAndAmortizationClient + interestClient + incomeExpenseSurplusDefecitClient
  
  const dashboardSurplusDefecit_Client = Number(data['ffa_dashboardSurplusDefecit_Client'][currentYear].value)
