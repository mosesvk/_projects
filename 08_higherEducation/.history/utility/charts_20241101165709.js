    ...
    chart: {
      ...
      events: {
        updated: function(chartContext, config) {
          ...
          console.log(config.config.series[4].data.length)
          if (config.config.series[4].data.length !== selectedYearsArray.length) {
            config.config.series.data[4].data = previousData // Update the previous data
            console.log('fixed', mainName, {currentData: config.config.series.data[4].data, prevData: previousData});
          }
        }
      }
    },
