...
const getTuitionDiscountRateChartOptions = (data) => {
  ...
  selectedYearsArray.map((year) => {
    ...
    const scholarshipsAndFinancialAidClient = Math.round(Number(data.revenueScholarshipsAndFinanancialAid_Client[year].value));
    ...
    const ratioPeer = Math.round(getAverageOfArray(data.ratio_Peer[year], 100));
    peerRatioArray.push(ratioPeer);
    const scholarshipsAndFinancialAidPeer = Math.round(getAverageOfArray(data.revenueScholarshipsAndFinanancialAid_Peer[year]));
    const tuitionAndFeesPeer = Math.round(
      getAverageOfArray(data.tuitionAndFees_Peer[year])
    );
    ...
    tuitionAndFeesRowClient.innerHTML += `
      <th scope="row" class="px-6 py-2 text-gray-900 whitespace-nowrap dark:text-white">
        ${formatCurrency(tuitionAndFeesRowClient)}
      </th>
    `;
    // Add year to table header
    tableHeaderRowPeer.innerHTML += `
