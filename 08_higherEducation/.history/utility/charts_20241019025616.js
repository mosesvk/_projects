...
const getDebtServiceCoverageChartOptions = (data) => {
  ...
  selectedYearsArray.map((year) => {
    const clientData = Number(data.ratio_Client[year].value * 100);
    const debtService = Number(data.debtService_Client[year].value);
    const interest = Number(data.interest_Client[year].value);
    const principalPayments = Number(data.principalPayments_Client[year].value);
    const totalOperatingRevenue = Number(data.totalOperatingRevenue_Client[year].value);
    ...
    principalPaymentsRow.innerHTML += `
      <th scope="row" class="px-6 py-2 text-gray-900 whitespace-nowrap dark:text-white">
        ${formatCurrency(principalPayments)}
      </th>
    `
