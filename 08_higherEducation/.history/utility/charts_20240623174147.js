...
const getFfaChartOptions = data => {
  ...
  const totalRevenueContributionsClient = Number (
    data['ffa_revenueContributions_Client'][currentYear].value
  );
  ...
  const dashboardSurplusDefecit_Client = Number (
    data['ffa_dashboardSurplusDefecit_Client'][currentYear].value
  );
  ...
  return {
    ...
    labels: ['Tuition & Fees', 'Scholarship & Financial Aid', 'Unrestricted Gfts', 'Auxiliary & Other', 'Restricted Gifts', 'Compensation & Benefits', 'General Expense', 'Surplus/Deficit'],
    responsive: [
      {
        breakpoint: 480,
        options: {
    ...
    xaxis: {
      labels: {
        ...
        rotate: -45, // Set the rotation angle (adjust as needed)
      },
    },
    legend: {
      horizontalAlign: 'center',
