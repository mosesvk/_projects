const urlUploadFile =
  "https://capincrouse.quickbase.com/db/bt3egnexu?a=API_EditRecord";
let uploadMainFile = "";
...
const getPngString = async (id, fieldId, fn) => {
...
//   console.log("string", string);
  ...
  uploadSingleToFile(fieldId, string, fn);
};

const mainPrint = async () => {
  uploadMainFile += "<qdbapi><apptoken>bpat4pgu9t69yby5gbemdbej52j</apptoken>";

  await getPngString("cfiRatio_chart", 896,"cfiOverall.png");
  await getPngString("cfi_primaryReserveRatio_chart", 892, "cfiPrimaryReserve.png");
  await getPngString("cfi_netIncomeOperationsRatio_chart", 895, "cfiNetIncome.png");
  await getPngString("cfi_returnOnNetAssets_chart", 894, "cfiNetAssets.png");
  await getPngString("cfi_viabilityRatio_chart", 893, "cfiViability.png");

  uploadMainFile += `<rid>${recordId}</rid></qdbapi>`;
...
function uploadSingleToFile(id, val, fn) {
  uploadMainFile += `<field fid='${id}' filename='${fn}'>${val}</field>`;
}
