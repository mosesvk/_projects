...
const getAdminCostsPerStudentChartOptions = (data) => {
  ...
  selectedYearsArray.map((year) => {
    ...
    array.map((item, idx) => {
      const salAdminAsst = getSumOfArray(data.salAdminAsst_Peer[year][idx]);
      const ficaAdminAsst = getSumOfArray(data.ficaAdminAsst_Peer[year][idx]);
      const healthAdminAsst = getSumOfArray(data.healthAdminAsst_Peer[year][idx]);
      const disabilityAdminAsst = getSumOfArray(
        data.disabilityAdminAsst_Peer[year][idx]
      );
      const retirementAdminAsst = getSumOfArray(
        data.retirementAdminAsst_Peer[year][idx]
      );
      const housingAdminAsst = getSumOfArray(data.housingAdminAsst_Peer[year][idx]);
      const otherAdminAsst = getSumOfArray(data.otherAdminAsst_Peer[year][idx]);
      const totalStudentFTE = getSumOfArray(data.totalStudentFte_Peer[year][idx]);
      const totalStudentUHC = getSumOfArray(data.totalStudentUhc_Peer[year][idx]);
      const peerNum =
      (salAdminAsst +
        ficaAdminAsst +
        healthAdminAsst +
    ...
    peerAvgArray.push(peerAvg);

    // const peer25 = get25thPercentileOfArray(peerData);
    // peer25Array.push(peer25);

    // const peer50 = getMidpointOfArray(peerData);
    // peer50Array.push(peer50);

    // const peer75 = get75thPercentileOfArray(peerData);
    // peer75Array.push(peer75);
  });
