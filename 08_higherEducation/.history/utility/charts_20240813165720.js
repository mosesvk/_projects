...
const getSalariesAndBenefitsPerNetTuitionChartOptions = (data) => {
  ...
  const numerator =
    Number(data["salariesAndWages_Client"][mostRecentYear]) +
    Number(data["employeeBenefits_Client"][mostRecentYear]);

  const denominator = Number(data["netTuitionAndFees_Client"][mostRecentYear]);

  const clientPercent = numerator/denominator

  console.log({ numerator, denominator, clientPercent });
