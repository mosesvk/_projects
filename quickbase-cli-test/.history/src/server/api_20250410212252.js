...
const {QuickbaseClient} = require('../db/QuickbaseClient')

// --- Debug: Check if .env variables are loaded ---
console.log("--- Environment Variable Check ---");
console.log("QB_USER_TOKEN:", process.env.QB_USER_TOKEN ? 'Loaded' : 'MISSING or empty');
console.log("QB_APP_ID:", process.env.QB_APP_ID ? 'Loaded' : 'MISSING or empty');
console.log("QB_TABLEID_INT_CLIENT:", process.env.QB_TABLEID_INT_CLIENT ? 'Loaded' : 'MISSING or empty');
console.log("----------------------------------");
// --- End Debug ---
...
class QuickbaseApiManager {
  ...
  async extractData(options) {
    ...
    if (!["client", "peer"].includes(type)) {
      throw new Error(
        `Invalid data type: ${type}. Must be 'client' or 'peer'.`
      );
    }
  ...
  async extractInternationalPeerData() {
    return this.extractData({
      type: "peer",
      tableId: process.env.QB_TABLEID_INT_PEER,
...
// Or use the existing methods
apiManager.extractInternationalClientData()
    .then(clientData => {
        console.log('Client Data:', clientData);
    });

// Accessing data store
window.dataStore = apiManager.getDataStore();
window.clientData = dataStore.getData('client');
window.peerData = dataStore.getData('peer');