...
const runApiMain = (recordsPeer, recordsClient) => {
  ...
  run_btn.addEventListener("click", () => {
    try {
      // Toggle button text and appearance to show loading spinner
      run_btn.innerHTML =
        ...
        Loading...`;
      run_btn.disabled = true; // Disable button during API call
      ...
      // Toggle button text and appearance back to "Run" after API call is finished
      run_btn.innerHTML = `<span class='text-xl mr-2'>RUN</span>
      <svg class="w-8 h-8 text-2xl  text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m7 16 4-4-4-4m6 8 4-4-4-4"/>
      </svg>`;
      run_btn.disabled = false; // Re-enable button
    } catch (err) {
      console.error(err);
