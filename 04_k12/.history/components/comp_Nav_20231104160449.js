...
document.addEventListener('click', (event) => {
  if (
    ...
    // Uncheck all checkboxes in the Select Years dropdown
    const yearCheckboxes = optionsList.querySelectorAll(
      'input[type="checkbox"]'
    );
...
const addUniqueYearsToOptionsSelectDropdown = (yearsArray) => {
  ...
  selectedYears_Array.length = 0
  console.log('in nav comp', localStorage.getItem('selectedYears'))
  ...
  yearsArray.forEach((year) => {
    ...
    newInput.addEventListener('change', (e) => changeListenerForInputYears(e.target, year));
