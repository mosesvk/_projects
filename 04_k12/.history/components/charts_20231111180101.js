const getMainChartOptions = (dataPeer, dataClient, numType, fixedNum = 0) => {
  ...
  const formatNumber = (value) => value.toLocaleString(); // Helper function for formatting numbers

  if (numType === 'dollar') {
    yaxisLabelFormatter = (value) => `$${formatNumber(value)}`;
    tooltipFormatter = (value) => `$${formatNumber(value.toFixed(fixedNum))}`;
  } else if (numType === 'percent') {
    yaxisLabelFormatter = (value) => `${formatNumber(value)}%`;
    tooltipFormatter = (value) => `${formatNumber(value.toFixed(fixedNum))}%`;
  } else {
    yaxisLabelFormatter = (value) => formatNumber(value); // No formatting for non-dollar and non-percent values
    tooltipFormatter = (value) => formatNumber(value.toFixed(fixedNum));
  }
