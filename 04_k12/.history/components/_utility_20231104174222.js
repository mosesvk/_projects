...
const getSelectedYearsFromLocalStorage = async () => {
  const storedSelectedYears = await JSON.parse(localStorage.getItem('selectedYears'));
...
const changeListenerForInputYears = (input, year) => {
  ...
  console.log('changeListenerForInputYears', selectedYearsArray)
...
const addUniqueYearsToOptionsSelectDropdown = (yearsArray) => {
  ...
  const storedYears = JSON.parse(localStorage.getItem('selectedYearArray'))
