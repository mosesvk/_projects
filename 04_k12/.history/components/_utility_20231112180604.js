...
const createDivChartandModal = (
  ...
  const modalComponent = `
    <div
      id=${modalId}
      class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:w-3/4 md:inset-0 h-full max-h-full flex"
      aria-modal="true"
      role="dialog"
...
const appendModalsToBody = (modalComponents, modalId) => {
  ...
  // Use requestAnimationFrame to ensure proper styling after the next frame
  requestAnimationFrame(() => {
    // Now, you can add any additional setup or styling if needed
    // ...
...
function addModalEventListeners(modalId) {
