...
const addToSingleRow = (selectedYears, name, client, peer, type, fixedNum, wa) => {
  ...
  addPeerDataToRow(tableReportRow, peer, type, fixedNum, wa, 'total');
};
...
const addPeerDataToRow = (tableRow, peer, type, fixedNum, wa, dataArray) => {
  console.log({tableRow, peer, type, fixedNum, wa, dataArray});
  const propClass =
    'px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white';
  const propScope = 'row';
