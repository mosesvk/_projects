const getWeightedAverageOfArray = (data, name) => {
  ...
  switch (name) {
    ...
    case 'receivableWriteOffsAsPercentNetTuitionAndFees':
        return receivableWriteOffsAsPercentNetTuitionAndFees_weightedAverage(data, name);
    default:
      return;
  }
};

const receivableWriteOffsAsPercentNetTuitionAndFees_weightedAverage = (data, name) => {
...
const netTuitionARasPercentCurrentAssets_weightedAverage = (data, name) => {
    ...
    let numStudentAccountsReceivable = getSumOfArray(data.studentAccountsReceivable[name]);
...
const propertyEquipmentPerStudent_weightedAverage = (data, name) => {
    ...
    let numLandBuildingsEquipmentNet = getSumOfArray(data.landBuildingsEquipmentNet[name]);
    let numLandAndLandImprovements = getSumOfArray(data.landAndLandImprovements[name]);
    let numStudentAverageEnrollment = getSumOfArray(data.studentAverageEnrollment_Main[name]);

    return (numLandBuildingsEquipmentNet - numLandAndLandImprovements) / numStudentAverageEnrollment
}

const netCashUsedOperating_overUnderBenchmark_weightedAverage = (data, name) => {
    let numCashFlowsOperatingActivities = getSumOfArray(data.cashFlowsOperatingActivities[name]);   
    let numTotalDepreciationExpense = getSumOfArray(data.totalDepreciationExpense[name]);

    return numNetnumCashFlowsOperatingActivitiesCashUsedOperating - numTotalDepreciationExpense
}
