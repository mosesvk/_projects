...
const addTableColumnsToReport = (tableHeader, years) => {
  // Find the table header row by its ID
  const tableHeaderRow = document.getElementById(tableHeader);

  // Find the first <th> element
  const firstTh = tableHeaderRow.querySelector('th');

  // Insert new year columns after the first <th>
  years.forEach((year) => {
    ...
    // Insert the new <th> element after the first <th>
    tableHeaderRow.insertBefore(newTh, firstTh.nextSibling);
  });
};
