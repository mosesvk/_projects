const getWeightedAverageOfArray = (name) => {
  ...
  switch (name) {
    ...
    case 'expendableReserves_inDays':
        return expendableReservesInDays_weightedAverage(parseData);
    default:
      return;
  }
};
...
const expendableReservesInDays_weightedAverage = (data) => {
    ...
    let numLandBuildingsEquipmentNet = getSumOfArray(data.landBuildingsEquipmentNet['total']);
    let numTotalDebt = getSumOfArray(data.totalDebt['total']);
    let numTotalDepreciationExpense = getSumOfArray(data.totalDepreciationExpense['total']);
};
