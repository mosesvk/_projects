...
const createDivChartandModal = (
  ...
  const modalComponent = `
    <div
      id=${modalId}
      tabindex="-1" class="hidden modal overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:w-3/4 md:inset-0 h-full max-h-full flex" aria-modal="true" role="dialog"
    >
...
const appendModalsToBody = (modalComponents) => {
  // Remove existing modals with tabindex="-1"
  const existingModals = document.querySelectorAll('div[tabindex="-1"]');
  existingModals.forEach((modal) => {
    document.body.removeChild(modal);
  });

  // Append new modal components to the end of the document body
  document.body.insertAdjacentHTML('beforeend', modalComponents);

  // Add a delay before styling (optional)
  setTimeout(() => {
    // For example, add event listeners to the newly created modals
    // Assuming modalId is part of your modalComponents, extract it and pass it to addModalEventListeners
    const modalId = extractModalId(modalComponents);
    addModalEventListeners(modalId);
  }, 0);
};

// You need to implement a function to extract modalId from modalComponents
function extractModalId(modalComponents) {
  // Implement the logic to extract modalId from modalComponents
  // ...

  // For example, assuming modalId is in the string
  return 'your-extracted-modal-id';
}
...
const addModalEventListeners = (modalId) => {
  ...
  const expandInfoButton = document.querySelector(`[data-modal-toggle="${modalId}"]`);
  const modal = document.getElementById(modalId);
  
  // Check if the elements exist before adding event listeners
  if (expandInfoButton && modal) {
    expandInfoButton.addEventListener('click', () => {
      console.log(`expand info - ${modalId}`)
