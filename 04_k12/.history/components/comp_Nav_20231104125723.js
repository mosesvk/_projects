...
const optionListRegion = document.getElementById('options-list-region');
...
customSelectRegion.addEventListener('click', (event) => {
  ...
  if (
    ...
    optionListRegion.classList.toggle('invisible');  // Corrected class name
  }
});
...
document.addEventListener('click', (event) => {
  if (
    ...
    !optionsList.contains(event.target) && !optionListRegion.contains(event.target) && customSelectRegion.contains(event.target)
  ) {
...
const addRegionsToOptionsSelectRegionDropdown = (regions_array) => {
  const optionsListRegion = document.getElementById('options-list-region');

  regions_array.forEach((regionData) => {
    const regionName = regionData[0];
    const states = regionData.slice(1);

    const newLabel = document.createElement('label');
    newLabel.setAttribute('for', `option-${regionName}`);
    newLabel.setAttribute(
      'class',
      'flex items-center justify-start px-4 py-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700'
    );
    ...
    const newInput = document.createElement('input');
    newInput.setAttribute('type', 'checkbox');
    newInput.setAttribute('id', `option-${regionName}`);
    newInput.setAttribute('class', 'form-checkbox h-4 w-4 text-gray-600 mr-2');
    newInput.setAttribute('value', regionName);
