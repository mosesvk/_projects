...
const addTableColumnsToReport = (tableHeader, yearsArray) => {
  ...
  yearsArray.forEach((year) => {
    ...
    newTh.setAttribute(
      'class',
      'px-6 py-3 transition duration-500 bg-yellow-100'
    );
    ...
    // To trigger the transition effect, we need to wait for the next repaint.
    // This can be done by accessing a computed style property like offsetWidth.
    newTh.style.offsetWidth;
    newTh.classList.remove('bg-yellow-100'); // Remove the background color after the transition
  });
};
