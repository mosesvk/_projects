...
const createAndAppendComponent = async (
  chartId,
  modalId,
  title,
  chartComponents,
  ...
  const updatedComponents = await createDivChartandModal(
    chartId,
    modalId,
    title,
    chartComponents,
...
const appendModalsToBody = (modalComponents, modalId) => {
  ...
  const existingModals = document.querySelectorAll(`[data-modal-container="${modalId}"]`);
