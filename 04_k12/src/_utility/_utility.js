...
const getPeerAndClientChartDataArrays = (
  ...
  safeYears.forEach((year) => {
    ...
    if (hasPeerData) {
      let avg;
      if (allData && typeof getWeightedAverageOfArray === "function") {
        const w = getWeightedAverageOfArray(allData, mainName, String(year));
        avg = w != null && Number.isFinite(Number(w)) ? Number(w) : parseFloat(getAverageOfArray(peerArray));
      } else {
