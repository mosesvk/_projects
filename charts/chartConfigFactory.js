...
class ChartConfigFactory {
  ...
  updateThemeColors() {
    ...
    this.themeColors = {
      ...
      // Add standard colors that were previously accessed directly
      standardColors: {
        green: window.chartColors.green,
        blue: window.chartColors.blue,
        orange: window.chartColors.orange,
        yellow: window.chartColors.yellow,
        purple: window.chartColors.purple,
        grey: window.chartColors.grey,
        red: window.chartColors.red,
        black: window.chartColors.black
      }
    };
  }
  ...
  createMainChartConfig({
    ...
    // Add safeguards for getting selected years
    let selectedYearsArray;
    try {
      selectedYearsArray = getSelectedYearsFromLocalStorage();
      // Ensure we have valid years - if not, try to extract them from the data
      if (!selectedYearsArray || !Array.isArray(selectedYearsArray) || selectedYearsArray.length === 0) {
        console.warn(`No years selected - attempting to extract years from data for ${mainName}`);
        
        // Try to extract years from client data if available
        if (typeof dataClient === 'object' && !Array.isArray(dataClient)) {
          const yearKeys = Object.keys(dataClient).filter(key => /^\d{4}$/.test(key));
          if (yearKeys.length > 0) {
            selectedYearsArray = yearKeys.sort();
            console.log(`Extracted years from client data: ${selectedYearsArray.join(', ')}`);
          }
        }
        
        // If still no years and we have parsedData with the client key
        if ((!selectedYearsArray || selectedYearsArray.length === 0) && 
            parsedData && typeof parsedData === 'object') {
          // Try to find years in the client data structure
          const clientDataKey = mainName === 'cfiRatio' ? 'cfiRatio_Client' : `${mainName}_Client`;
          if (parsedData[clientDataKey]) {
            const yearKeys = Object.keys(parsedData[clientDataKey]).filter(key => /^\d{4}$/.test(key));
            if (yearKeys.length > 0) {
              selectedYearsArray = yearKeys.sort();
              console.log(`Extracted years from parsed client data: ${selectedYearsArray.join(', ')}`);
            }
          }
        }
        
        // If still no years, use default years
        if (!selectedYearsArray || selectedYearsArray.length === 0) {
          ...
          console.log(`Using default years: ${selectedYearsArray.join(', ')}`);
        }
      }
    } catch (error) {
      console.error(`Error getting selected years: ${error.message}`);
      // Default to last 4 years if there's an error
      const currentYear = new Date().getFullYear();
      selectedYearsArray = [
        (currentYear - 3).toString(),
        (currentYear - 2).toString(),
    ...
    if (mainName == 'cfiRatio') console.log("createMainChartConfig()", {
      dataPeer,
      dataClient,
      numType,
      fixedNum,
  ...
  switch (selectedYearsLength) {
    ...
    if (mainName == "cfiRatio") {
      console.log('Creating CFI Ratio chart config:', {
        dataPeer,
        dataClient,
        benchmark
      });
      
      // Make sure the clientArray and peerAvg data is defined and valid
      if (!clientArray || !clientArray.length) {
        console.warn("Client data for CFI Ratio is missing or empty");
        // Try to extract directly from the data source
        if (typeof dataClient === 'object' && !Array.isArray(dataClient)) {
          try {
            clientArray = Object.keys(dataClient)
              .filter(key => /^\d{4}$/.test(key))
              .sort()
              .map(year => {
                if (dataClient[year] && 'value' in dataClient[year]) {
                  return parseFloat(dataClient[year].value);
                }
                return null;
              })
              .filter(val => val !== null);
            console.log("Extracted client data directly:", clientArray);
          } catch (e) {
            console.error("Error extracting client data:", e);
            clientArray = selectedYearsArray.map(() => 1); // Fallback to dummy data
          }
        } else {
          clientArray = selectedYearsArray.map(() => 1); // Fallback to dummy data
        }
      ...
      if (!peerAvg || !peerAvg.length) {
        console.warn("Peer data for CFI Ratio is missing or empty");
        // Default to approximating peer data based on client data if available
        if (clientArray && clientArray.length) {
          peerAvg = clientArray.map(val => val ? val * 0.9 : 1);
          peerMid = clientArray.map(val => val ? val * 0.85 : 0.9);
          peer25 = clientArray.map(val => val ? val * 0.75 : 0.8);
          peer75 = clientArray.map(val => val ? val * 0.95 : 1.1);
        } else {
          peerAvg = selectedYearsArray.map(() => 0.9);
          peerMid = selectedYearsArray.map(() => 0.85);
          peer25 = selectedYearsArray.map(() => 0.8);
          peer75 = selectedYearsArray.map(() => 1.1);
        }
      }
      
      // Create annotation for benchmark and zones
      const annotations = {
        yaxis: [{
          y: -4,
          borderColor: '#ff6384',
      ...
      // Create series array with proper data
      const series = [
        {
      ...
      return {
        mainName: "cfiRatio", // Add the mainName to the config for reference
        ...
        yaxis: {
          ...
          axisBorder: {
            show: true,
            color: this.themeColors.chartColors.labelColor
          },
          labels: {
      ...
      console.log('Creating chart config for:', mainName);
    }
    
    // if (mainName == "doeOverall") {
    //   const data = JSON.parse(localStorage.doeData);
    //   let doeOverall_annotation = [
    //     {
    //       id: "annotation",
    //       y: benchmark,
    ...
    return {
      ...
      dataLabels: {
        ...
        background: {
          enabled: true,
          foreColor: this.themeColors.standardColors.green,
          padding: 4,
          borderRadius: 2,
          borderWidth: 1,
          borderColor: "#ffffff",
  ...
  createLineChartConfig({
    ...
    if (mainName == "cfiRatio") {
      console.log("createLineChartConfig() cfiRatio", {
        dataPeer,
        dataClient,
        parsedData,
      });
  ...
  createFunctionalAllocationConfig({
    ...
    try {
      ...
      // Define series colors using theme colors
      const seriesColors = [
        this.themeColors.standardColors.blue,
        this.themeColors.standardColors.orange,
        this.themeColors.standardColors.red,
        this.themeColors.standardColors.black,
      ];
  ...
  createCostOfContributionsConfig({
    ...
    const seriesColors = [
      this.themeColors.standardColors.blue,
      this.themeColors.standardColors.green,
      this.themeColors.standardColors.red,
      this.themeColors.standardColors.grey,
    ];
    ...
    return {
      ...
      markers: {
        ...
        colors: [null, null, this.themeColors.standardColors.red, this.themeColors.standardColors.grey],
        strokeWidth: 2,
      },
      xaxis: {
        categories: selectedYearsArray,
