...
class ChartConfigFactory {
    ...
    createLineChartConfig({
      ...
      return {
        ...
        dataLabels: {
          ...
          formatter: formatters.dataLabelFormatter,
          textAnchor: "middle",
          offsetY: -10,
          style: {
            fontSize: "12px",
    ...
    createFunctionalAllocationConfig({
      ...
      try {
          ...
          selectedYearsArray.forEach((year, index) => {
            if (parsedData["functionalExpensePercent_program_Client"][year]) {
              const value = parseFloat(
                parsedData["functionalExpensePercent_program_Client"][year].value * 100
              );
        ...
        if (parsedData["functionalExpensePercent_administrative_Client"]) {
          selectedYearsArray.forEach((year, index) => {
            if (parsedData["functionalExpensePercent_administrative_Client"][year]) {
              const value = parseFloat(
                parsedData["functionalExpensePercent_administrative_Client"][year].value * 100
              );
        ...
        if (parsedData["functionalExpensePercent_fundraising_Client"]) {
          selectedYearsArray.forEach((year, index) => {
            if (parsedData["functionalExpensePercent_fundraising_Client"][year]) {
              const value = parseFloat(
                parsedData["functionalExpensePercent_fundraising_Client"][year].value * 100
              );
        ...
        // Calculate peer average for program expenses
    ...
    createCostOfContributionsConfig({
      ...
      const safeMaxDollarValue = allDollarValues.length > 0 ? Math.max(...allDollarValues) * 1.1 : 1000000;
      ...
      const safeMaxRatioValue = allRatioValues.length > 0 ? Math.max(...allRatioValues) * 1.2 : 0.3;
      ...
      selectedYearsArray.forEach((year) => {
        if (
          ...
          netAssetsWithoutDRData.push(
            Number(parsedData["netAssetsWithoutDonorRestrictions_Client"][year].value)
          );
        } else {
      ...
      selectedYearsArray.forEach((year) => {
        if (
          ...
          netAssetsWithDRData.push(
            Number(parsedData["netAssetsWithDonorRestrictionsSum_Client"][year].value)
          );
        } else {
