...
class ChartManager {
  ...
  populateModalContent(
    ...
    selectedYears.forEach((year, index) => {
      ...
      if (chartData.clientArray && chartData.clientArray[index] !== undefined) {
        ...
        // Add client data to row - pass the raw value, not formatted value
      ...
      if (chartData.peerAvg && chartData.peerAvg[index] !== undefined) {
        // Format and add peer data - pass raw values, not formatted values
  ...
  // In chartManager.js - Update the _createPeerDataCell method
  ...
  _createPeerDataCell(row, value, dataType, fixedNum) {
    ...
    if (value !== undefined && value !== null) {
      // Get the numeric value - handle case where value might already be a number
      const numValue = typeof value === 'number' ? value : parseFloat(value);
  
      // Format the value based on type using styleNumber
      let formattedValue;
      if (!isNaN(numValue) && typeof styleNumber === "function") {
        // Use styleNumber with the proper parameters
  ...
  _addClientDataToModalRow(yearRow, clientValue, type, fixedNum) {
    ...
    if (clientValue !== undefined && clientValue !== null) {
      // Get the numeric value - handle case where value might already be a number
      const numValue = typeof clientValue === 'number' ? clientValue : parseFloat(clientValue);
  
      // Format the value
      const formattedValue = !isNaN(numValue) && typeof styleNumber === "function"
        ? styleNumber(numValue, type, fixedNum)
        : (numValue.toFixed(fixedNum || 2));
