...
class ChartSystem {
      ...
      document.addEventListener("dark-mode", () => {
        ...
        Object.keys(this.chartInstances).forEach(chartId => {
          if (this.chartInstances[chartId] && this.chartConfigs[chartId]) {
            this.chartInstances[chartId].updateOptions(this.chartConfigs[chartId]);
          }
        });
      });
    }
    ...
    updateCashFlowModal(mainName, data, [financing, investing, operating, total]) {
      ...
      selectedYears.forEach(year => {
        const row = this._createCashFlowDataRow(mainName, year, operatingData, investingData, financingData, totalData);
    ...
    _createYearRow(mainName, year) {
      ...
      yearRow.className = "bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600";
    ...
    _createCashFlowDataRow(mainName, year, operatingData, investingData, financingData, totalData) {
      const row = document.createElement("tr");
      row.className = "bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600";
      row.id = `${mainName}_modal_${year}`;
  
      // Common cell class
      const cellClass = "px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white opacity-75 justify-between border-r-2 dark:border-gray-600";
