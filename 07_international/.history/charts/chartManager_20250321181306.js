...
class ChartManager {
  ...
  updateModal(mainName, peerData, clientData, parsedData, dataType, fixedNum) {
    ...
    if (!modal) {
      console.warn(`Modal element with ID "${mainName}_modal" not found - trying to create it`);
      this.ensureModalExists(mainName);
      return;
    }
    ...
    if (modal && (!modal.querySelector('table') || !modal.querySelector('thead') || !headerRow)) {
      console.warn(`Modal ${mainName}_modal has incomplete structure - recreating it`);
      ...
      if (!headerRow) {
        console.error(`Failed to find header row in recreated modal for ${mainName}`);
        return;
      }
    }
