...
class ExcelReportGenerator {
  constructor() {
    ...
    this.fieldMappings = [
      ...
      ["daysExpensesInUnrestrictedNA_excludingPPE", [9, 47, 85, 123], false, false, "cash"],
      ["daysExpensesInNAwithDR", [10, 48, 86, 124], false, false, "cash"],
      ["daysExpensesInNAwithDR_excludingPPE", [11, 49, 87, 125], false, false, "cash"],
      ...
      ["percentWithoutDR_excludingPPE", [21, 59, 97, 135], false, false, "asset"],
      ["percentWithoutDR", [22, 60, 98, 136], false, false, "asset"],
      
      // Income data
      ["netIncomeRatio", [23, 61, 99, 137], false, false, "income"],
      ["contributionsTrend_basedOnNumberOfDonors", [24, 62, 100, 138], false, false, "income"],
      ["contributionsTrend", [25, 63, 101, 139], false, false, "income"],
      ["contributionsPercentWithoutDR", [26, 64, 102, 140], false, false, "income"],
      ["contributionsPercentWithDR", [27, 65, 103, 141], false, false, "income"],
      ["contributionsPerGivingUnit", [28, 66, 104, 142], false, false, "income"],
      ["contributionsPerMissionaryUnit", [29, 67, 105, 143], false, false, "income"],
      ["contributionsPerFullTimeEquivalent", [30, 68, 106, 144], false, false, "income"],
      ["fundraisingAsPercentOfContributions", [31, 69, 107, 145], false, false, "income"],
      ["annualizedInvestmentReturn", [32, 70, 108, 146], false, false, "income"],
      
      // Expense data
      ["functionalExpensePercent_program", [33, 71, 109, 147], false, false, "expense"],
      ["functionalExpensePercent_administrative", [34, 72, 110, 148], false, false, "expense"],
      ["functionalExpensePercent_fundraising", [35, 73, 111, 149], false, false, "expense"],
      ["costOfContributions", [37, 75, 113, 151], false, false, "expense"],
      ["expensesPerGivingUnit", [38, 76, 114, 152], false, false, "expense"],
      ["expensesPerMissionaryUnit", [39, 77, 115, 153], false, false, "expense"],
      ["expensesPerFullTimeEquivalent", [40, 78, 116, 154], false, false, "expense"],
      ["salariesAndBenefitsAsPercentOfTotalExpenses", [41, 79, 117, 155], false, false, "expense"],
      ["salariesAndBenefitsPerFTE", [42, 80, 118, 156], false, false, "expense"],
      
      // Misc data
      ["percentageAssessmentOnRestrictedGifts", [43, 81, 119, 157], false, false, "misc"],
    ];
...
prepareAllFieldData() {
  try {
    ...
    this.fieldMappings.forEach((mapping, index) => {
      ...
      let dataObject;
      switch(category) {
        case "general": dataObject = generalData; break;
        case "cash": dataObject = cashData; break;
        case "asset": dataObject = assetData; break;
        case "income": dataObject = incomeData; break;
        case "expense": dataObject = expenseData; break;
        case "misc": dataObject = miscData; break;
        default: return; // Skip if no valid category
      }
  ...
  async createPrintExcel() {
    ...
    try {
      ...
      // ...rest of client information code...
  
      // Process all metrics
      this.prepareAllFieldData();
  
      // IMPORTANT: Make sure XML is properly closed
      // Add the column list tag if it's not already there
      if (!this.xmlPayload.includes("<clist>")) {
        this.xmlPayload += this.XML.COLUMN_LIST;
      }
      
      // Add the closing tag if it's not already there
      if (!this.xmlPayload.includes("</qdbapi>")) {
        this.xmlPayload += this.XML.FOOTER;
      }
      ...
      success: function (response) {
        try {
          ...
          if (errorCode === "0") {
            ...
            console.log("Successfully uploaded to QuickBase, Record ID:", recordId);
          ...
          console.error("Error parsing QuickBase response:", parseError, "Response:", response);
