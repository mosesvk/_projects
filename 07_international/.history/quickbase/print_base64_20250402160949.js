...
async function processChartsWithSpacing(chartMappings) {
  ...
  for (let i = 0; i < chartMappings.length; i++) {
    ...
    try {
      ...
      // Temporarily modify the chart element to ensure full capture
      chartElement.style.height = '450px'; // Increase height to capture full chart
      ...
      const apexChart = window[chartId] || chartManager.getChart(chartId);
      
      // If it's an ApexChart, ensure it's properly sized
      if (apexChart && typeof apexChart.updateOptions === 'function') {
        const currentOptions = apexChart.opts || {};
        // Store the original height
        const originalHeight = currentOptions.chart && currentOptions.chart.height;
        
        // Update the chart height to be larger
        ...
        // Give the chart time to update its dimensions
      ...
      // Restore original ApexChart height if we changed it
      if (apexChart && typeof apexChart.updateOptions === 'function' && 
          currentOptions && currentOptions.chart && originalHeight) {
        apexChart.updateOptions({
          chart: {
            ...currentOptions.chart,
