...
const mainPrint = async () => {
  ...
  try {
    ...
    uploadPresentationFile += uploadSinglePresentationToFile(169, uniqueClients.size);
    ...
    for (let i = 0; i < chartMappings.length; i += batchSize) {
      ...
      const batchResults = await Promise.all(batch.map(async ({ chartId, fieldId }) => {
        try {
      ...
      batchResults.forEach(result => {
        if (result && result.base64String) {
          uploadPresentationFile += uploadSinglePresentationToFile(result.fieldId, result.base64String);
        }
      });
    }
    ...
    if (xmlUpload.find("qdbapi").find("errcode").text() == "0") {
      ...
      createToastWarning(`Quickbase returned an error: ${xmlUpload.find("qdbapi").find("errtext").text() || 'Unknown error'}`);
    }
  } catch (error) {
    console.error("Error in mainPrint:", error);
    createToastWarning("Error creating presentation: " + (error.statusText || error.message || "Unknown error"));
  } finally {
