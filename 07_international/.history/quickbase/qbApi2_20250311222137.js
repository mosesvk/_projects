...
class AppController {
  ...
  // Initialize event listeners
  initializeEventListeners() {
    // The beforeunload event is fine to keep here
    window.addEventListener("beforeunload", () => {
      localStorage.clear();
    });
...
// Keep these original event listeners

// Client initialization needs to happen on DOM content loaded directly
document.addEventListener("DOMContentLoaded", () => {
  // Initialize dropdown options from global arrays
...
// Initialize the application controller when the window loads
window.onload = () => {
