...
(function() {
  ...
  // Override with enhanced version that ensures proper event flow
  SystemConnector.prototype.handleRunButtonClick = async function() {
    console.log("Enhanced handleRunButtonClick called - ensuring chart rendering");
    ...
    try {
      ...
      // Force chart rendering if it hasn't happened automatically
      setTimeout(() => {
        if (this.chartsRendered < this.totalExpectedCharts) {
          console.log("Charts did not render automatically, forcing display");
          ...
          // Display charts using available methods
          ...
          // Update global chart references
          if (typeof updateGlobalChartReferences === "function") {
            updateGlobalChartReferences();
          }
          
          // Explicitly trigger charts rendered event
          document.dispatchEvent(new Event('chartsRendered'));
          
          // Also trigger data processing complete for report component
          document.dispatchEvent(new CustomEvent('dataProcessingComplete'));
        }
      }, 1500); // Give charts enough time to render automatically first
    } catch (error) {
      console.error("Error in enhanced handleRunButtonClick:", error);
    }
  };
  
  // Ensure we don't have duplicate run button listeners
  document.addEventListener("DOMContentLoaded", function() {
    const runButton = document.querySelector("#run");
    if (runButton) {
      // Create a clean copy without event listeners
      const newRunButton = runButton.cloneNode(true);
      runButton.parentNode.replaceChild(newRunButton, runButton);
      
      // Add our single consistent listener
      newRunButton.addEventListener("click", function() {
        if (window.systemConnector) {
          window.systemConnector.handleRunButtonClick();
        }
      });
      
      console.log("Run button event handler unified");
    }
  });
  
  // Enhance chart system to ensure report component gets updated
  document.addEventListener("chartsRendered", function() {
    console.log("Charts rendered event received - updating report");
    
    // Slight delay to ensure all charts are fully processed
    setTimeout(() => {
      // Trigger report display
      if (typeof displayReportComponent === "function") {
        console.log("Explicitly calling displayReportComponent");
        displayReportComponent();
      } else if (window.reportComponent && typeof window.reportComponent.displayReportComponent === "function") {
        console.log("Calling reportComponent.displayReportComponent");
...
SystemConnector.prototype.displayCharts = function() {
  console.log("Enhanced displayCharts method called");
  
  try {
    // First check if necessary data exists in localStorage
    ...
    if (!hasData) {
      console.warn("No chart data available, cannot display charts");
      if (typeof createToastWarning === "function") {
        createToastWarning("No data available to display charts. Please try again.");
      }
      return;
    }
    
    // Force chart initialization with the enhanced function if available
    if (typeof window.initializeChartDisplay === "function") {
      console.log("Using enhanced chart initialization");
      window.initializeChartDisplay();
    }
    // Fallback to component display functions
    else if (
      typeof displayComponents === "object" && 
      typeof displayComponents.displayAllComponents === "function"
    ) {
      ...
      console.error("No chart display functions available");
      if (typeof createToastWarning === "function") {
        createToastWarning("Error displaying charts: No display functions available");
      }
      return;
    }
    
    // Explicitly trigger the chartsRendered event
    setTimeout(() => {
      console.log("Explicitly dispatching chartsRendered event");
      document.dispatchEvent(new Event('chartsRendered'));
      
      // Explicitly call displayReportComponent
      setTimeout(() => {
        if (typeof displayReportComponent === "function") {
          console.log("Explicitly calling displayReportComponent");
          displayReportComponent();
...
SystemConnector.prototype.handleRunButtonClick = async function() {
  console.log("Fixed handleRunButtonClick called");
  ...
  try {
    ...
    // CRITICAL FIX: Force chart display with explicit calls
    console.log("Forcing chart display with explicit component calls");
    
    // First attempt with displayComponents
    if (window.displayComponents && typeof displayComponents.displayAllComponents === "function") {
      console.log("Using displayComponents.displayAllComponents");
      displayComponents.displayAllComponents();
    } 
    // Fallback to individual component functions
    else if (
      typeof displayGeneralComponent === "function" &&
      typeof displayCashComponent === "function" &&
      typeof displayIncomeComponent === "function" &&
    ...
    // Explicitly trigger chartsRendered event
    console.log("Explicitly dispatching chartsRendered event");
    document.dispatchEvent(new Event('chartsRendered'));
    
    // Explicitly call displayReportComponent with delay
    setTimeout(() => {
      if (typeof displayReportComponent === "function") {
        console.log("Explicitly calling displayReportComponent");
        displayReportComponent();
    ...
    this.isLoading = false;
    console.log("handleRunButtonClick complete");
