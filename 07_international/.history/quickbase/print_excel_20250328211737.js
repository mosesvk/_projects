...
const ExcelReportGenerator = (() => {
  ...
  function dataArrayObjects(
    ...
    const filterNumeric = (arr) =>
      arr.filter((item) => !isNaN(parseFloat(item)));
  ...
  async function createPrintExcel() {
    ...
    try {
      ...
      const yearArray =
        selectedYears_Set instanceof Set ? Array.from(selectedYears_Set) : [];
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.dispatchEvent(new CustomEvent('quickbase_functions_ready'));
  console.log("QuickBase Excel report functions are now available");
});
