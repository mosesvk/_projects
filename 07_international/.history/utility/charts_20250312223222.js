...
// Apply at least 30% padding to ratio values
const minRatioValue = Math.floor(Math.min(...allRatioValues) * 0.7); // 30% padding below min
const maxRatioValue = Math.ceil(Math.max(...allRatioValues) * 1.3); // 30% padding above max

// Calculate dynamic min and max for dollar values (left y-axis)
// Apply at least 30% padding to dollar values
const minDollarValue = Math.floor(Math.min(...allDollarValues) * 0.7); // 30% padding below min
const maxDollarValue = Math.ceil(Math.max(...allDollarValues) * 1.3); // 30% padding above max
