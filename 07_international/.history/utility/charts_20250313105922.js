...
const getFunctionalAllocationChartOptions = (
  ...
  try {
    ...
    // Update modal with data - wrapping in try/catch to prevent blocking chart rendering if modal update fails
    try {
      selectedYearsArray.forEach((year, index) => {
        const tableModalRow = document.getElementById(`${mainName}_modal_${year}`);
        if (tableModalRow) {
          // Add data for program expenses
          if (programData && programData.clientArray) {
            addClientDataToModalRow(
              tableModalRow,
              programData.clientArray[index],
              numType,
              fixedNum,
              "Program"
            );
          ...
          if (adminData && adminData.clientArray) {
            addClientDataToModalRow(
              tableModalRow,
              adminData.clientArray[index],
              numType,
              fixedNum,
              "Administrative"
            );
          ...
          if (fundraisingData && fundraisingData.clientArray) {
            addClientDataToModalRow(
              tableModalRow,
              fundraisingData.clientArray[index],
              numType,
              fixedNum,
              "Fundraising"
            );
          ...
          if (programData && programData.peerAvg) {
            addPeerDataToModalRow(
              tableModalRow,
              programData.peerAvg[index],
              0,
              0,
              0,
              "Program Peer Avg",
    ...
    // Create a safe array for each series to ensure it's valid for ApexCharts
    // Convert each value to a number and handle any nulls or undefineds
    const createSafeArray = (dataObj, property) => {
      if (!dataObj || !dataObj[property] || !Array.isArray(dataObj[property])) {
        return Array(selectedYearsArray.length).fill(null);
      }
      
      return dataObj[property].map(value => {
    ...
    // Create safe arrays for each series
    const programSeriesData = createSafeArray(programData, 'clientArray');
    const adminSeriesData = createSafeArray(adminData, 'clientArray');
    const fundraisingSeriesData = createSafeArray(fundraisingData, 'clientArray');
    const peerAvgSeriesData = createSafeArray(programData, 'peerAvg');

    // Log the series data for debugging
    console.log("Chart series data (safe arrays):", {
    ...
    // Create the final chart options object
    const chartOptions = {
      colors: seriesColors,
      series: [
        {
    ...
    // Final sanity check on chart options
    if (!chartOptions.series.every(series => Array.isArray(series.data))) {
      throw new Error("Not all series data are arrays");
    }
