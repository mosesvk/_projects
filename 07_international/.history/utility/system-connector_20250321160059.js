...
class SystemConnector {
    ...
    async handleRunButtonClick() {
      ...
      try {
        ...
        // Fetch and process data using AppController if available
    ...
    async processDataWithAppController(selectedYears) {
      ...
      const recordsPeer = await appController.apiService.getRecordsForPeer(selectedYears);
      countUniqueClients(recordsPeer);
  
      const recordsClient = await appController.apiService.getRecordsForClient(selectedYears);
    ...
    async processDataLegacy(selectedYears) {
      // Implement a fallback method that mimics the old approach
      ...
      // Get records for peer
      const recordsPeer = await getRecordsForPeer(selectedYears);
      if (recordsPeer) {
        countUniqueClients(recordsPeer);
      }
      
      // Get records for client
      const recordsClient = await getRecordsForClient(selectedYears);
