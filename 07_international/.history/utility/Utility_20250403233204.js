...
function populateModalContent(headerRow, selectedYears, clientData, peerData, parsedData, type, fixedNum, wa) {
  ...
  selectedYears.forEach((year) => {
    ...
    if (peerData && peerData[year]) {
      ...
      if (wa === "wa" && typeof getWeightedAverageOfArray === "function" && parsedData) {
        try {
          // Calculate weighted average for this specific chart and year
          const weightedAvg = getWeightedAverageOfArray(parsedData, mainName, year);
          
          // For other percentiles, use regular calculations
          const peerValues = peerData[year];
          const peerMid = Array.isArray(peerValues) ? getMidpointOfArray(peerValues, mainName) : 0;
          const peer25 = Array.isArray(peerValues) ? get25thPercentileOfArray(peerValues, mainName) : 0;
          const peer75 = Array.isArray(peerValues) ? get75thPercentileOfArray(peerValues, mainName) : 0;
          
          // For percentage type, multiply values by 100 (except for annualizedInvestmentReturn which is already handled)
          let multiplier = dataProcessingType === "percent" && !isAnnualizedInvestmentReturn ? 100 : 1;
          ...
          const peerValues = peerData[year];
          const peerAvg = Array.isArray(peerValues) ? getAverageOfArray(peerValues) : 0;
          const peerMid = Array.isArray(peerValues) ? getMidpointOfArray(peerValues, mainName) : 0;
          const peer25 = Array.isArray(peerValues) ? get25thPercentileOfArray(peerValues, mainName) : 0;
          const peer75 = Array.isArray(peerValues) ? get75thPercentileOfArray(peerValues, mainName) : 0;
          
          // Add the peer data to the row
          addPeerDataToModalRow(yearRow, peerAvg, peerMid, peer25, peer75, dataProcessingType, fixedNum);
        }
      } else {
