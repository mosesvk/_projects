...
function updateModal(mainName, peerData, clientData, parsedData) {
  ...
  if (!modal) {
    ...
    console.log('Possible modals found:', Array.from(possibleModals).map(m => m.id));
    return;
  }
  ...
  if (!headerRow) {
    console.warn(`Header row with selector "${headerRowSelector}" not found in modal ${modalSelector}`);
    ...
    if (fallbackHeaderRow) {
      ...
      populateModalContent(fallbackHeaderRow, selectedYears, clientData, peerData);
      return;
    }
    return;
  }
...
function populateModalContent(headerRow, selectedYears, clientData, peerData) {
  ...
  selectedYears.forEach(year => {
    ...
    if (peerData && peerData[year]) {
      ...
      const peerMid = Array.isArray(peerValues) ? getMidpointOfArray(peerValues) : 0;
      const peer25 = Array.isArray(peerValues) ? get25thPercentileOfArray(peerValues) : 0;
      const peer75 = Array.isArray(peerValues) ? get75thPercentileOfArray(peerValues) : 0;
...
// Helper functions for addModalColumns, createYearRow, etc.
// These would be implementations of your existing helper functions
