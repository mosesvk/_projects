...
function clientMatchesFilters(
  ...
  // If no types OR no areas selected, return false immediately
  ...
  const areaMatch =
    ...
    clientData.areaQuery.some((areaName) => {
      ...
      return selectedAreas.some((areaCode) => {
        const areaObj = window.areas_Array.find(
          (area) => area.str === areaCode
        );
...
function updateClientDropdownBasedOnFilters() {
  // First, check if we're being called from Utility.js when Header.js is active
  if (window.headerJsActive && new Error().stack.includes("Utility.js")) {
    console.log("Blocking Utility.js call when Header.js is active");
    return;
  }
  ...
  if (
    window.selectedTypes_Array.size === 0 ||
    window.selectedAreas_Array.size === 0
  ) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.removeEventListener("filtersChanged", updateClientDropdownBasedOnFilters);
document.addEventListener("filtersChanged", updateClientDropdownBasedOnFilters);
  ...
  function initializeFilterTriggers() {
    ...
    typeCheckboxes.forEach((checkbox) => {
      checkbox.addEventListener("change", function () {
        ...
        // Directly call the function instead of triggering an event
        updateClientDropdownBasedOnFilters();
      });
    });
