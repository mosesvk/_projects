...
class ReportComponent {
  ...
  initializeEventListeners() {
    ...
    if (generateReportsBtn) {
      generateReportsBtn.addEventListener("click", this.handleGenerateReportClick.bind(this));
    }
    
    // Listen for data processing complete
    document.addEventListener("dataProcessingComplete", this.onDataProcessingComplete.bind(this));
  }
...
insertDataToReport(data, selectedYears, metricsConfig) {
  ...
  for (const metricConfig of metricsConfig) {
    ...
    if (clientData) {
      selectedYears.forEach((year) => {
        ...
        if (value !== undefined && Number(value) !== 0) {
          // Format the value
          const formattedValue = this.styleNumber(value, dataType, fixedDecimals, metricName);
    ...
    if (peerData) {
      ...
      // Format values
      const textAvg = this.styleNumber(avg, dataType, fixedDecimals, metricName);
      const textQ1 = this.styleNumber(q1, dataType, fixedDecimals, metricName);
      const textMedian = this.styleNumber(median, dataType, fixedDecimals, metricName);
      const textQ3 = this.styleNumber(q3, dataType, fixedDecimals, metricName);
      
      // Add formatted cells
      const cells = [
        {
          value: textAvg,
          class: "px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white border-r-2 dark:border-gray-600",
        },
        {
          value: textQ1,
          class: "px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white border-r-2 dark:border-gray-600",
...
window.displayReportComponent = reportComponent.displayReportComponent.bind(reportComponent);
});