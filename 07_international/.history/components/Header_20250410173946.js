...
/**
 * Update all input elements with values from global variables
 * This is specifically for slider input elements, not display elements
 */
updateAllInputs() {
  // Update slider inputs from global variables
  const sliderInputs = {
  ...
  Object.entries(sliderInputs).forEach(([inputId, globalVar]) => {
    const inputElement = document.getElementById(inputId);
    if (inputElement) {
      inputElement.value = window[globalVar];
    }
  });
  
  // Also update displays to keep everything in sync
  this.updateAllDisplays();
}
}
