...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const handleSliderChange = function(sliderType, value) {
    switch(sliderType) {
      case 'givingMin':
        window.sliderValue = parseInt(value) || 0;
        break;
      case 'givingMax':
        window.sliderValue2 = parseInt(value) || 25000;
        break;
      case 'missionMin':
        window.missionValue = parseInt(value) || 0;
        break;
      case 'missionMax':
        window.missionValue2 = parseInt(value) || 10000;
        break;
    }
...
// Add mutation observer to catch dynamic changes
document.addEventListener("DOMContentLoaded", function() {
  const observeSliderChanges = (selector, sliderType) => {
  ...
  // Observe each slider
  observeSliderChanges('#givingUnitsMin', 'givingMin');
  observeSliderChanges('#givingUnitsMax', 'givingMax');
  observeSliderChanges('#missionUnitsMin', 'missionMin');
  observeSliderChanges('#missionUnitsMax', 'missionMax');
});

// Fallback event listeners
['#givingUnitsMin', '#givingUnitsMax', '#missionUnitsMin', '#missionUnitsMax'].forEach(selector => {
  const element = document.querySelector(selector);
  if (element) {
    element.addEventListener('change', function() {
      const sliderType = this.id === 'givingUnitsMin' ? 'givingMin' :
                         this.id === 'givingUnitsMax' ? 'givingMax' :
                         this.id === 'missionUnitsMin' ? 'missionMin' :
                         'missionMax';
      ...
      handleSliderChange(sliderType, this.value);
    });
  }
});
