const displayReportComponent = () => {
  ...
  try {
    ...
    console.log("Report tables found:", Object.entries(reportTables).map(([name, el]) => 
      `${name}: ${el ? "Found" : "Missing"}`).join(", "));
...
function validateReportStructure() {
  ...
  const reportStructure = {
    generalDataTable: ['givingUnits', 'missionaryUnit', 'numberOfEmployeesFTE', 'itExpenses'],
    cashDataTable: [
    ...
    assetDataTable: ['percentWithDR', 'percentWithoutDR_excludingPPE', 'percentWithoutDR'],
    incomeDataTable: [
...
const insertDataToReport = (data, selectedYears, arrayOfNames) => {
  ...
  for (let metricConfig of arrayOfNames) {
    ...
    console.log(`Found row for metric: ${metricName}`);
    ...
    if (clientData) {
      console.log(`Found client data for ${metricName}:`, clientData);
      ...
      selectedYears.forEach(year => {
        ...
        cell.scope = "row";
        cell.className = "px-6 py-4 text-xl font-medium text-gray-900 whitespace-nowrap dark:text-white opacity-80 justify-between border-r-2 dark:border-gray-600";
        
        if (value !== undefined && Number(value) !== 0) {
          // Format the value
          const formattedValue = styleNumber(value, dataType, fixedDecimals, metricName);
      ...
      console.warn(`No client data found for ${metricName}`);
    ...
    if (peerData) {
      ...
      if (weightedAvg === "wa" && typeof getWeightedAverageOfArray === 'function') {
        avg = getWeightedAverageOfArray(data, metricName) || 0;
      } else if (peerData[dataArray] && Array.isArray(peerData[dataArray])) {
        avg = getAverageOfArray(peerData[dataArray], metricName) || 0;
      }
      
      // Calculate percentiles
      if (peerData[dataArray] && Array.isArray(peerData[dataArray])) {
        if (typeof calculatePercentiles === 'function') {
          try {
            [q1, median, q3] = calculatePercentiles(peerData[dataArray], dataType, fixedDecimals);
          } catch (error) {
            console.warn(`Error calculating percentiles for ${metricName}:`, error);
          }
        } else {
          // Fallback calculation methods
          q1 = get25thPercentileOfArray(peerData[dataArray], metricName) || 0;
          median = getMidpointOfArray(peerData[dataArray], metricName) || 0;
          q3 = get75thPercentileOfArray(peerData[dataArray], metricName) || 0;
        }
      }
      ...
      const textMedian = styleNumber(median, dataType, fixedDecimals, metricName);
      const textQ3 = styleNumber(q3, dataType, fixedDecimals, metricName);
      
      // Add formatted cells
      const cells = [
        { value: textAvg, class: "px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white border-r-2 dark:border-gray-600" },
        { value: textQ1, class: "px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white border-r-2 dark:border-gray-600" },
        { value: textMedian, class: "px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white border-r-2 dark:border-gray-600" },
        { value: textQ3, class: "px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white border-r-2 dark:border-gray-600" }
      ];
      ...
      // Add empty cells for peer stats
      for (let i = 0; i < 4; i++) {
        const emptyCell = document.createElement("th");
        emptyCell.scope = "row";
