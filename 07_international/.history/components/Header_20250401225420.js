...
function overrideUtilityFilterFunctions() {
  // Set this to completely block Utility.js functions
  window.utilityFunctionsDisabled = true;

  // More aggressive override
  window.updateClientSelectionBasedOnFilters = function () {
    if (window.utilityFunctionsDisabled) {
      console.log("Utility function completely disabled");
      return;
    }
  };
  ...
  // If these exist on the global document object, remove them
  if (typeof document.updateClientSelectionBasedOnFilters === "function") {
    delete document.updateClientSelectionBasedOnFilters;
  }

  if (typeof document.updateClientDropdownBasedOnFilters === "function") {
    delete document.updateClientDropdownBasedOnFilters;
  }
}
...
function clientMatchesFilters(
  ...
  // Check area match - Convert area codes to area names for comparison
  // If no areas selected, assume nothing matches for areas
  let areaMatch = false;
  if (selectedAreas.length > 0) {
    areaMatch = clientData.areaQuery && 
      Array.isArray(clientData.areaQuery) && 
      clientData.areaQuery.some(areaName => {
        // Find if any selected area includes this area name
        return selectedAreas.some(areaCode => {
          const areaObj = window.areas_Array.find(area => area.str === areaCode);
  ...
  // Check type match
  // If no types selected, assume nothing matches for types
  let typeMatch = false;
  ...
  // If both selectedTypes and selectedAreas are empty, nothing should match
  if (selectedTypes.length === 0 && selectedAreas.length === 0) {
    return false;
  }
  // If only selectedTypes is empty but areas are selected, only check area match
  else if (selectedTypes.length === 0) {
    return givingUnitMatch && missionUnitMatch && areaMatch;
  }
  // If only selectedAreas is empty but types are selected, only check type match
  else if (selectedAreas.length === 0) {
    return givingUnitMatch && missionUnitMatch && typeMatch;
  }
  // Otherwise check both type and area matches
  else {
...
function updateClientDropdownBasedOnFilters() {
  ...
  // Clear the current selection set first
  window.selectedClients_Array.clear();
...
document.addEventListener("DOMContentLoaded", function () {
  // First, remove ALL filtersChanged event listeners
  ...
  function initializeFilterTriggers() {
    ...
    areaCheckboxes.forEach((checkbox) => {
      // Clone the checkbox to remove all existing event listeners
      ...
      newCheckbox.addEventListener("change", function() {
        ...
        // Directly call our filter function
        updateClientDropdownBasedOnFilters();
      });
    });
    ...
    typeCheckboxes.forEach((checkbox) => {
      ...
      newCheckbox.addEventListener("change", function() {
        ...
        console.log(`Type ${typeValue} ${this.checked ? "selected" : "deselected"}`);
    ...
    if (selectAllArea) {
      ...
      const newSelectAllArea = selectAllArea.cloneNode(true);
      selectAllArea.parentNode.replaceChild(newSelectAllArea, selectAllArea);
      
      newSelectAllArea.addEventListener("change", function() {
        const isChecked = this.checked;
        const areaCheckboxes = document.querySelectorAll(
          "#options-list-area input[type='checkbox']"
        );
    ...
    if (selectAllType) {
      // Clone to remove existing listeners
      const newSelectAllType = selectAllType.cloneNode(true);
      selectAllType.parentNode.replaceChild(newSelectAllType, selectAllType);
      
      newSelectAllType.addEventListener("change", function() {
        const isChecked = this.checked;
        const typeCheckboxes = document.querySelectorAll(
          "#options-list-type input[type='checkbox']"
        );
    ...
    console.log("Global Variables State:", {
      ...
      selectedClients: {
        ...
        size: window.selectedClients_Array
          ...
          : "N/A",
        firstFive: window.selectedClients_Array
  ...
  document.removeEventListener(
    "filtersChanged",
    window.updateClientSelectionBasedOnFilters
  );
  document.removeEventListener(
    "filtersChanged",
    window.updateClientDropdownBasedOnFilters
  );
  ...
  setTimeout(function() {
    // Reinitialize the filter triggers with our version
    initializeFilterTriggers();
    
    // Apply filters once to initialize
    updateClientDropdownBasedOnFilters();
    
    console.log("Filter triggers reinitialized with direct handlers");
  }, 1000); // Delay to ensure other initialization is complete
});
