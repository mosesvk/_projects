      ...
      events: {
        mounted: async function (chartContext, config) {
          setTimeout(function () {
            ...
            // Calculate the actual chart area width (accounting for padding/margins)
            // The actual plotting area is typically smaller than the container
            const chartArea = document.querySelector(
              `#${chartId} .apexcharts-inner`
            );
            const plotWidth = chartArea
              ? chartArea.getBoundingClientRect().width
              : chartWidth - 40;
            console.log("plotWidth", plotWidth);

            // Create a completely new annotation
            const newAnnotation = {
              id: "annotation",
              y: benchmark,
              strokeDashArray: 0,
              ...
              strokeWidth: 1,
              width: `${plotWidth}px`,
              // Try positioning the line to match plot area
              offsetX: 0,
              label: {
                text: "Benchmark",
                borderColor: "transparent",
            ...
            // First remove any existing annotation
            cfiRatio_chart.clearAnnotations();

            // Then apply the new one
            cfiRatio_chart.addYaxisAnnotation(newAnnotation);
          }, 600);
        },
      },
    },
