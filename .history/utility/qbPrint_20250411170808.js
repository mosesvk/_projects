// qbPrint.js - Updated version with enhanced functionality from printBase64.js

/**
 * Enhanced chart processing for export that ensures the full chart is captured
 *
...
async function processChartsWithSpacing(chartMappings) {
  ...
  for (let i = 0; i < chartMappings.length; i++) {
    ...
    try {
      ...
      // Get the ApexChart instance if available
...
async function sendToQuickbase(xml) {
  try {
    const response = await $.ajax({
      ...
      url: "https://capincrouse.quickbase.com/db/buk93bd7x?a=API_AddRecord", // Preserving original endpoint
      dataType: "xml",
      processData: false,
      data: xml,
...
/**
 * Enhanced version of mainPrint using ApexCharts export functionality
 */
async function enhancedMainPrint() {
  ...
  try {
    // Show all sections that might be hidden for proper chart rendering
    ...
    // Define all chart IDs and their corresponding field IDs - kept from original qbPrint.js
    const chartMappings = [
    ...
    // Prepare XML for Quickbase
    let uploadXml = "<qdbapi><apptoken>c3qhvhmcgbwze7hwbiavcm3hnmc</apptoken>";
    
    // Add client information - using same field IDs from original qbPrint.js
    uploadXml += createFieldXml(31, clientName);
    uploadXml += createFieldXml(32, uniqueClients.size);
